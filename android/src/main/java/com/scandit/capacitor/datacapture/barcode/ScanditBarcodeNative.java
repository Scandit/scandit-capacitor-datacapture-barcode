...
public class ScanditBarcodeNative extends Plugin implements Emitter {
  ...
  public void createBarcodeArView(PluginCall call) {
    ...
    // BarcodeAr manages its own camera (like SparkScan) and the JS sample fires
    // camera.switchToDesiredState(On) immediately after connectToElement resolves,
    // so the view must only be created (and the call only resolved) once the user
    // has granted the camera permission — otherwise the camera-on call races the
    // OS dialog and the user sees a black preview on first launch.
    if (!checkCameraPermission()) {
      requestPermissionForAlias("camera", call, "onBarcodeArCameraPermissionResult");
      return;
    }
    attachBarcodeArView(call);
  }

  @SuppressWarnings("unused")
  @PermissionCallback
  private void onBarcodeArCameraPermissionResult(PluginCall call) {
    if (!checkCameraPermission()) {
      call.reject("Camera permissions not granted.");
      return;
    }
  ...
  private void attachBarcodeArView(PluginCall call) {
    final String finalViewJson = call.getData().getString("viewJson");
    mainThread.runOnMainThread(
        () -> {
          ...
          barcodeArModule.addViewToContainer(container, finalViewJson, new NoopFrameworksResult());
